describe('attendance report', () => {
  function seedTimeTable() {
    ...
    cy.wait(1000)
    // Define reusable GraphQL queries and mutations
    const LIST_STUDENT_SERVICE_QUERY = `
    ...
    // Define a type for attendance record configuration
    ...
    // Define attendance configurations to apply
    const attendanceConfigs: { [key: string]: AttendanceConfig } = {
      MONDAY: {
        common: {
          datetime: '2025-01-06T05:05:28Z',
          draftAt: '2025-01-06T05:05:28Z',
          note: null,
          checked: false,
          attachments: [],
          timetableId: morningTimetables[0]?.id as '',
        },
        items: [
          {
            type: 'ABSENT',
      ...
      TUESDAY: {
        common: {
          datetime: '2025-01-07T05:05:28Z',
          draftAt: '2025-01-07T05:05:28Z',
          note: null,
          checked: false,
          attachments: [],
          timetableId: morningTimetables[1]?.id as '',
        },
        items: [
          {
            type: 'PRESENT',
      ...
      WEDNESDAY: {
        common: {
          datetime: '2025-01-08T05:05:28Z',
          draftAt: '2025-01-08T05:05:28Z',
          note: null,
          checked: false,
          attachments: [],
          timetableId: morningTimetables[2]?.id as '',
        },
        items: [
          {
            type: 'ABSENT',
    ...
    const updateAttendanceRecords = (
      dayConfig: AttendanceConfig,
      timetableId: string,
      date: string,
    ) => {
      cy.seedGraphqlData(
        LIST_STUDENT_SERVICE_QUERY,
        {
          timetableId: timetableId,
          filterDate: date,
        },
      ).then((data) => {
        // @ts-expect-error - Ignoring possible undefined result from GraphQL query since we provide a fallback empty array
        const results = data?.listStudentServiceWithAttendance?.results || []

        // Update attendance records for each student
        results.forEach((result: { attendance: { id: string, studentServiceId: string } }, index: number) => {
          // Skip if we don't have a configuration for this index
          if (index >= dayConfig.items.length) return

          const attendanceConfig = dayConfig.items[index]
          const attendanceId = result.attendance.id
          const studentServiceId = result.attendance.studentServiceId
    ...
    updateAttendanceRecords(
      attendanceConfigs.MONDAY as AttendanceConfig,
      morningTimetables[0]?.id as '',
      '2025-01-06',
    )

    updateAttendanceRecords(
      attendanceConfigs.TUESDAY as AttendanceConfig,
      morningTimetables[1]?.id as '',
      '2025-01-07',
    )

    updateAttendanceRecords(
      attendanceConfigs.WEDNESDAY as AttendanceConfig,
      morningTimetables[2]?.id as '',
      '2025-01-08',
    )
  }
  ...
  it('should display attendance report', () => {
    cy.visit('/attendances/reports').wait(3000)

    cy.setBearerToken()

    seedTimeTable()
    cy.get('a[href="/attendances/reports"]').click().wait(3000)
    ...
    cy.get('[data-testid="date-range"] .el-range-input')
      .first()
      .click()
      .type('ច, 06/01/2025', { delay: 50 })
      .tab()
      .type('អា, 12/01/2025{enter}', { delay: 50 })
    cy.get('body').click(0, 0).wait(300)

    cy.dataGridSearch('attendance', 'NAK VANNA')
    cy.get('tr[aria-rowindex="1"] td[aria-colindex="5"]')
      .should('have.text', '2')

    cy.get('tr[aria-rowindex="1"] td[aria-colindex="6"]')
      .should('have.text', '0')

    cy.dataGridSearch('attendance', 'ROEURN YEN')
    cy.get('tr[aria-rowindex="1"] td[aria-colindex="5"]')
      .should('have.text', '1')
    ...
    cy.get('tr[aria-rowindex="1"] td[aria-colindex="7"]')
      .should('have.text', '1 time(s) 15 minute(s)')

    cy.dataGridSearch('attendance', 'Test Enrollment')
    cy.get('tr[aria-rowindex="1"] td[aria-colindex="5"]')
      .should('have.text', '0')
    ...
    cy.get('tr[aria-rowindex="1"] td[aria-colindex="7"]')
      .should('have.text', '3 time(s) 27 minute(s)')
    cy.wait(1000)
  })
})
