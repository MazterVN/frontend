...
import type { CreateRelatedRelativeInput, EditRelativeQuery } from '#gql'
import { RelativeCreate, RelativeEdit } from '#components'
import type { FormSubmitEvent } from '#ui/types'
import { Language } from '#gql/default'

export const useRelativeStore = defineStore('relativeStore', () => {
    type Relative = At<EditRelativeQuery, 'editRelative'>
    type RelativeItem = At<EditRelativeQuery, 'editRelative.relatives'>
    const modal = useModalStore()
    const grid = ref<Grid | null>(null)
    const item = ref<Relative>(initItem())
    ...
    function initChild(language: Language): RelativeItem {
      return {
        id: uuidv4(),
        name: '',
        language: language,
    ...
    function toInput(data: Relative): CreateRelatedRelativeInput {
      return {
        id: data.id,
        name: data.name,
        language: data.language,
        sortOrder: data.sortOrder,
        relatives: data.relatives.map(item => ({
          id: item.id,
          name: item.name,
      ...
      switch (toolbarId) {
        case 'add':
          modal.open(RelativeCreate, {
            ...
            onSubmit: async (
              evt: FormSubmitEvent<Relative>,
            ) => {
              const result = await relativeRepository.createRelated(
                toInput(evt.data),
              )
              if (!result?.createRelatedRelative.errors.length) {
        ...
        case 'edit':
          modal.open(RelativeEdit, {
            fullscreen: false,

            onInit: async () => {
              resetItem()
              const data = await relativeService.edit(id)
              if (data?.editRelative) {
                item.value = data.editRelative
              }
            },
            onSubmit: async (
              evt: FormSubmitEvent<Relative>,
            ) => {
              const result = await relativeRepository.updateRelated(id, toInput(evt.data))
              if (!result?.updateRelatedRelative.errors.length) {
    ...
    const gridConfig = (): GridModel => ({
      ...globalGridConfig,
      dataSource: gridDataSource(ListRelativeDocument),
