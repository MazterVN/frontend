...
import type { CreateRelatedGradeLevelInput, EditGradeLevelQuery } from '#gql'
import { GradeLevelCreate, GradeLevelEdit } from '#components'
import type { FormSubmitEvent } from '#ui/types'
import { Language } from '#gql/default'

export const useGradeLevelStore = defineStore('gradeLevelStore', () => {
    type GradeLevel = At<EditGradeLevelQuery, 'editGradeLevel'>
    type GradeLevelItem = At<EditGradeLevelQuery, 'editGradeLevel.gradeLevels'>
    const modal = useModalStore()
    const grid = ref<Grid | null>(null)
    const item = ref<GradeLevel>(initItem())
    ...
    function initChild(language: Language): GradeLevelItem {
      return {
        id: uuidv4(),
        name: '',
        language: language,
    ...
    function toInput(data: GradeLevel): CreateRelatedGradeLevelInput {
      return {
        id: data.id,
        name: data.name,
        language: data.language,
        sortOrder: data.sortOrder,
        gradeLevels: data.gradeLevels.map(item => ({
          id: item.id,
          name: item.name,
      ...
      switch (toolbarId) {
        case 'add':
          modal.open(GradeLevelCreate, {
            ...
            onSubmit: async (
              evt: FormSubmitEvent<GradeLevel>,
            ) => {
              const result = await gradeLevelRepository.createRelated(
                toInput(evt.data),
              )
              if (!result?.createRelatedGradeLevel.errors.length) {
        ...
        case 'edit':
          modal.open(GradeLevelEdit, {
            fullscreen: false,

            onInit: async () => {
              resetItem()
              const data = await gradeLevelService.edit(id)
              if (data?.editGradeLevel) {
                item.value = data.editGradeLevel
              }
            },
            onSubmit: async (
              evt: FormSubmitEvent<GradeLevel>,
            ) => {
              const result = await gradeLevelRepository.updateRelated(id, toInput(evt.data))
              if (!result?.updateRelatedGradeLevel.errors.length) {
    ...
    const gridConfig = (): GridModel => ({
      ...globalGridConfig,
      dataSource: gridDataSource(ListGradeLevelDocument),
