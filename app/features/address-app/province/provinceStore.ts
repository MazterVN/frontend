...
import type { CreateRelatedProvinceInput, EditProvinceQuery } from '#gql'
import { ProvinceCreate, ProvinceEdit } from '#components'
import type { FormSubmitEvent } from '#ui/types'
import { Language } from '#gql/default'

export const useProvinceStore = defineStore('provinceStore', () => {
    type Province = At<EditProvinceQuery, 'editProvince'>
    type ProvinceItem = At<EditProvinceQuery, 'editProvince.provinces'>
    const modal = useModalStore()
    const grid = ref<Grid | null>(null)
    const item = ref<Province>(initItem())
    ...
    function initChild(language: Language): ProvinceItem {
      return {
        id: uuidv4(),
        name: '',
        language: language,
    ...
    function toInput(data: Province): CreateRelatedProvinceInput {
      return {
        id: data.id,
        name: data.name,
      ...
      switch (toolbarId) {
        case 'add':
          modal.open(ProvinceCreate, {
            ...
            onSubmit: async (
              evt: FormSubmitEvent<Province>,
            ) => {
              const result = await provinceRepository.createRelated(
                toInput(evt.data),
              )
              if (!result?.createRelatedProvince.errors.length) {
        ...
        case 'edit':
          modal.open(ProvinceEdit, {
            fullscreen: false,

            onInit: async () => {
              resetItem()
              const data = await provinceService.edit(id)
              if (data?.editProvince) {
                item.value = data.editProvince
              }
            },
            onSubmit: async (
              evt: FormSubmitEvent<Province>,
            ) => {
              const result = await provinceRepository.updateRelated(id, toInput(evt.data))
              if (!result?.updateRelatedProvince.errors.length) {
    ...
    const gridConfig = (): GridModel => ({
      ...globalGridConfig,
      dataSource: gridDataSource(ListProvinceDocument),
