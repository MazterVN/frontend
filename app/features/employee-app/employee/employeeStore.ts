import type {
  Grid,
  GridModel,
  RowDeselectEventArgs,
  RowSelectEventArgs,
} from '@syncfusion/ej2-vue-grids'
import * as v from 'valibot'
import type {
  ClickEventArgs,
  MenuEventArgs,
} from '@syncfusion/ej2-navigations'
import { v4 as uuidv4 } from 'uuid'
import { EmployeeCreate, EmployeeEdit } from '#components'
import type { FormSubmitEvent } from '#ui/types'
import {
  EmployeeRole,
  Gender,
  type CreateUpsertEmployeeInput,
  type EditEmployeeQuery,
} from '#gql/default'

export const useEmployeeStore = defineStore('employeeStore', () => {
    type Employee = At<EditEmployeeQuery, 'editEmployee'>
    const modal = useModalStore()
    const grid = ref<Grid | null>(null)
    const item = ref<Employee>(initItem())

    const validationSchema = v.object({
      id: v.pipe(v.string(), v.nonEmpty()),
      name: v.pipe(
        v.string('Please enter name'),
        v.nonEmpty('Please enter name'),
      ),
      latin: v.nullable(v.string('Please enter latin')),
      gender: v.pipe(
        v.string('Please select gender'),
        v.nonEmpty('Please select gender'),
      ),
      dob: v.pipe(
        v.string('Please select date of birth'),
        v.nonEmpty('Please select date of birth'),
      ),
      type: v.pipe(
        v.string('Please select type'),
        v.nonEmpty('Please select type'),
      ),
      tel: v.pipe(
        v.string('Please enter tel'),
        v.nonEmpty('Please enter tel'),
      ),
    })
    const setGridRef = (component: GridComponent | null): void => {
      grid.value = component?.ej2Instances ? component?.ej2Instances : null
    }
    function initToolbar() {
      grid.value?.toolbarModule.enableItems(['edit'], false)
      grid.value?.contextMenuModule.contextMenu.enableItems(['Edit'], false)
    }
    function resetItem() {
      item.value = initItem()
    }
    function initItem(): Employee {
      return {
        id: uuidv4(),
        name: '',
        latin: '',
        gender: Gender.FEMALE,
        dob: '',
        type: EmployeeRole.TEACHER,
        tel: '',
      }
    }
    function refreshGrid() {
      grid.value?.refresh()
    }

    function toInput(item: Employee): CreateUpsertEmployeeInput {
      return {
        id: item.id,
        name: item.name,
        latin: item.latin,
        gender: item.gender,
        dob: item.dob,
        type: item.type,
        tel: item.tel,
      }
    }

    function handleToolbarClick(args: ClickEventArgs): void {
      const selectedRecords = grid.value?.getSelectedRecords() as Employee[]
      const toolbarId = args.item.id as 'add' | 'edit' | 'delete'
      switch (toolbarId) {
        case 'add':
          modal.open(EmployeeCreate, {
            fullscreen: false,
            ui: {
              width: 'w-full sm:max-w-4xl',
            },

            onInit: async () => {
              resetItem()
            },
            onSubmit: async (evt: FormSubmitEvent<Employee>) => {
              const result = await employeeRepository.bulkCreateUpsert(
                toInput(evt.data),
              )
              if (
                result
                && !result?.createUpsertEmployee.errors.length
              ) {
                showSuccessToast('បន្ថែមបានជោគជ័យ')
                resetItem()
                refreshGrid()
              }
            },

          })
          break

        case 'edit':
          modal.open(EmployeeEdit, {
            fullscreen: false,
            ui: {
              width: 'w-full sm:max-w-4xl',
            },

            onInit: async () => {
              resetItem()
              const id = selectedRecords[0]?.id ?? ''
              const data = await employeeService.edit(id)
              if (data.editEmployee) {
                item.value = data.editEmployee
              }
            },
            onSubmit: async (evt: FormSubmitEvent<Employee>) => {
              const result = await employeeRepository.bulkCreateUpsert(
                toInput(evt.data),
              )
              if (
                result
                && !result?.createUpsertEmployee.errors.length
              ) {
                showSuccessToast('កែប្រែបានជោគជ័យ')
                refreshGrid()
              }
            },

          })

          break

        case 'delete':
          console.log('Performing delete operation')
          // Perform delete operation logic here
          break

        default:
          console.log('Unknown operation')
          // Handle unknown operation
          break
      }
    }

    function handleContextMenuClick(args: MenuEventArgs): void {
      const toolbarId = args.item.id as 'edit'
      switch (toolbarId) {
        case 'edit':
          break

        default:
          console.log('Unknown operation')
          // Handle unknown operation
          break
      }
    }

    function handleRowSelect(_args: RowSelectEventArgs): void {
      const selectedRecords = grid.value?.getSelectedRecords()
      grid.value?.toolbarModule.enableItems(
        ['edit'],
        selectedRecords?.length === 1,
      )
      grid.value?.contextMenuModule.contextMenu.enableItems(
        ['Edit'],
        selectedRecords?.length === 1,
      )
    }

    function handleRowDeselect(_args: RowDeselectEventArgs): void {
      const selectedRecords = grid.value?.getSelectedRecords()
      grid.value?.toolbarModule.enableItems(
        ['edit'],
        selectedRecords?.length === 1,
      )
      grid.value?.contextMenuModule.contextMenu.enableItems(
        ['Edit'],
        selectedRecords?.length === 1,
      )
    }
    const gridConfig = (): GridModel => ({
      ...globalGridConfig,
      dataSource: gridDataSource(ListEmployeeDocument),
      contextMenuItems: toContextMenuItems([
        {
          id: 'edit',
          text: 'Edit',
          iconCss: 'e-menu-icon e-icons e-edit',
        },
        'AutoFit',
        'AutoFitAll',
        'SortAscending',
        'SortDescending',
        'FirstPage',
        'PrevPage',
        'LastPage',
        'NextPage',
      ]),
      toolbarClick: handleToolbarClick,
      contextMenuClick: handleContextMenuClick,
      rowSelected: handleRowSelect,
      rowDeselected: handleRowDeselect,
      toolbar: [
        {
          text: 'បន្ថែម',
          prefixIcon: 'icon-[heroicons--plus-20-solid] !w-4 !h-4',
          id: 'add',
        },
        {
          text: 'កែ',
          prefixIcon:
                    'icon-[heroicons--pencil-square-16-solid] !w-4 !h-4',
          id: 'edit',
        },
        'Search',
      ],
    })
    return {
      gridConfig,
      setGridRef,
      initToolbar,
      item,
      validationSchema,
    }
})
