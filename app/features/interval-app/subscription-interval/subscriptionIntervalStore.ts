...
import { SubscriptionIntervalCreate, SubscriptionIntervalEdit } from '#components'
import type { FormSubmitEvent } from '#ui/types'
import {
  IntervalMode,
  type CreateUpsertSubscriptionIntervalInput,
  type EditSubscriptionIntervalQuery,
} from '#gql/default'

export const useSubscriptionIntervalStore = defineStore(
  'subscriptionIntervalStore',
  () => {
        type SubscriptionInterval = At<
          EditSubscriptionIntervalQuery,
          'editSubscriptionInterval'
        >
        const modal = useModalStore()
        const grid = ref<Grid | null>(null)
        const item = ref<SubscriptionInterval>(initItem())
        ...
        function initItem(): SubscriptionInterval {
          return {
            id: uuidv4(),
            day: 0,
        ...
        function handleToolbarClick(args: ClickEventArgs): void {
          const selectedRecords
                = grid.value?.getSelectedRecords() as SubscriptionInterval[]
          const toolbarId = args.item.id as 'add' | 'edit' | 'delete'
          switch (toolbarId) {
            case 'add':
              modal.open(SubscriptionIntervalCreate, {
                fullscreen: false,
                ui: {
                ...
                onSubmit: async (
                  evt: FormSubmitEvent<SubscriptionInterval>,
                ) => {
                  const result
                                = await subscriptionIntervalRepository.createUpsert(
                                  toInput(evt.data),
                                )
                  if (
                    result
                    && !result?.createUpsertSubscriptionInterval.errors
                      .length
                  ) {
            ...
            case 'edit':
              modal.open(SubscriptionIntervalEdit, {
                fullscreen: false,
                ...
                onInit: async () => {
                  ...
                  const result
                                = await subscriptionIntervalRepository.edit(id)
                  if (result?.editSubscriptionInterval) {
                    item.value = result.editSubscriptionInterval
                  }
                },
                onSubmit: async (
                  evt: FormSubmitEvent<SubscriptionInterval>,
        ...
        const gridConfig = (): GridModel => ({
          ...globalGridConfig,
          dataSource: gridDataSource(ListSubscriptionIntervalDocument),
