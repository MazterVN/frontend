        ...
        <template #checked="{ data }">
          <div class="flex justify-center">
            <UCheckbox
              ...
              :data-testid="`attendance-${data.index}-checked`"
              @update:model-value="(_value) => {
                debounceCallback(() => store.attendanceChanged(data.attendance, 'checked'));
              }"
            />
          </div>
        </template>
      ...
      >
        <template #isAbsent="{ data }">
          <UCheckbox
            ...
            :data-testid="`attendance-${data.index}-is-absent`"
      ...
      >
        <template #type="{ data }">
          <DropdownList
            ...
            :data-testid="`attendance-${data.index}-type`"
            @update:model-value="(_value) => {
      ...
      >
        <template #lateTime="{ data }">
          <UInput
            ...
            :data-testid="`attendance-${data.index}-late-time`"
            :disabled="data.attendance.type != AttendanceType.PRESENT_LATE"
            min="0"
            max="60"
            type="number"
            @update:model-value="(_value) => debounceCallback(() => store.attendanceChanged(data.attendance, 'lateTime'))"
          />
        </template>
      </EColumn>
      <EColumn
      ...
      >
        <template #note="{ data }">
          <UTextarea
            ...
            :data-testid="`attendance-${data.index}-note`"
            resize
            :rows="2"
            @update:model-value="(_value) => {
